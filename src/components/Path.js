...
export default function Path({ pages }) {
    const currentPage = pages[pages.length - 1].name
    return (
        <div className="flex w-full">
            <div className="items-center flex">
                {pages.map((pageData) => {
                    if (pageData.name !== currentPage)
                        return (
                            <>
                                <Link to={`/${pageData.link}`}><button className="hover:underline mr-2 font-bold text-[14px] leading-[20px] tracking-[0.005em] text-[#BC6C25]">{pageData.name}</button></Link>
                        ...
                        return <p className="font-bold text-[14px] leading-[20px] tracking-[0.005em] text-customGrey">{pageData.name}</p>
                })}
            </div>
        </div>
    )
...
// list = [ {name: "Dashboard", link: "dashboard"}] Link to={'/'}