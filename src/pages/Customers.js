...
export default function CustomersPage() {
  ...
  const lastPage = totalItems
    ? totalItems.length % itemsPerPage === 0
      ? totalItems.length / itemsPerPage
      : Math.floor(totalItems.length / itemsPerPage) + 1
    : null;
  // const shownItems = ;
  ...
  useEffect(() => {
    getAllUsers()
      .then((data) => data)
      .then((data) => setTotalItems(data.data.data.users));
  }, []);
        ...
        <>
          ...
          <div className="mt-5 flex w-full items-center">
            <div className="flex border border-[#E0E2E7] bg-white rounded-lg p-[2px]">
              ...
              <TabButton
                handleSelect={() => handleSelectTabButton("active")}
                isSelected={activePage === "active"}
                disabled={true}
              >
                Active
              </TabButton>
              <TabButton
                handleSelect={() => handleSelectTabButton("blocked")}
                isSelected={activePage === "blocked"}
                disabled={true}
              >
                Blocked
              </TabButton>
          ...
          <div className="rounded-b-xl w-full p-4 items-center flex">
            ...
            <div className="ml-auto flex space-x-2">
              ...
              {arr.map((pageNum) => {
                return (
                  <StyledDashboardButton
                    handleClick={() => setCurrentPage(pageNum)}
